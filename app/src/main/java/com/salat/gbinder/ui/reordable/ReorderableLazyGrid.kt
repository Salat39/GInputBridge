 ...
 * @param scrollMoveMode The [com.salat.gbinder.ui.reordable.ScrollMoveMode] that will be used to determine how other items move around when an item is dragged to the top or bottom, or start or end of the grid. See [ScrollSwapMode](sh.calvin.reorderable.ScrollSwapMode).
 * @param onMove The function that is called when an item is moved. Make sure this function updates the UI grid synchronously and returns only after the in-memory items are moved. Do not perform database, disk, or network work here. Persist the final order after drop instead. This suspend function is invoked with the `rememberReorderableLazyGridState` scope, allowing for lightweight async processing when strictly necessary. Note that the scope used here is the one provided by the composition where `rememberReorderableLazyGridState` is called, for long running work that needs to outlast `rememberReorderableLazyGridState` being in the composition you should use a scope that fits the lifecycle needed.
 */
@Composable
fun rememberReorderableLazyGridState(
    ...
    val orientation by remember(lazyGridState) { derivedStateOf { lazyGridState.layoutInfo.orientation } } // decide by main axis

    val state = remember(
        scope,
        lazyGridState,
        scrollThresholdPx,
        absoluteScrollThresholdPadding,
        scroller,
        orientation,
        scrollMoveMode,
        layoutDirection,
        ...
        ReorderableLazyGridState(
            ...
            // decide movement when the dragging rect crosses the target's center along main axis
            shouldItemMove = when (orientation) {
                Orientation.Vertical -> { draggingRect: Rect, itemRect: Rect ->
                    itemRect.center.y in draggingRect.top..<draggingRect.bottom
                }

                Orientation.Horizontal -> { draggingRect: Rect, itemRect: Rect ->
                    itemRect.center.x in draggingRect.left..<draggingRect.right
                }
            }
        )
    }
 ...
 * @param enabled Whether or this item is reorderable. If true, the item will not move for other items but may still be draggable. To make an item not draggable, set `enable = false` in [com.salat.gbinder.ui.reordable.draggable] instead.
