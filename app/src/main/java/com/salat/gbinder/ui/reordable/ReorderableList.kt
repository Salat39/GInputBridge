...
class ReorderableListState internal constructor(
    ...
    /**
     * Reset transient drag state when the backing dataset or layout contract changes.
     * This prevents stale offsets from leaking into a logically different list.
     */
    internal fun reset() {
        draggingItemIndex = null
        animatingItemIndex = null
        itemOffsets.forEach { animatable ->
            scope.launch {
...
fun <T> ReorderableColumn(
    ...
    val reorderableListState = remember(list.size, spacing, layoutDirection, coroutineScope, onMove, onSettle) {
    ...
    LaunchedEffect(list.toList(), spacing, layoutDirection) {
        reorderableListState.reset()
    }
